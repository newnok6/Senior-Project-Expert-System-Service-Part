...
public class RevisionDialog extends JDialog {
	...
	private void configureFrame()
	{
		...
		image = new JLabel();
		image.setIcon(new ImageIcon(FileIO.findFile("main/java/jcolibri/examples/TravelRecommender/gui/step5.png")));
...
//		Lay out the panel.
	...
	void showCase()
	{
		...
		PizzaDescription desc = (PizzaDescription) _case.getDescription();
		
		//this.accommodation.setSelectedItem(desc.getAccommodation().toString());
	//	this.duration.setValue(desc.getDuration());
		this.holidayType.setSelectedItem(desc.getTypeOfpizza());
		...
		PizzaSolution sol = (PizzaSolution) _case.getSolution();
	
		this.hotel.setText(sol.getPizza());
	}
	...
	//	desc.setDuration(this.duration.getNumber().intValue());
		desc.setTypeOfpizza((String)this.holidayType.getSelectedItem());
		...
		sol.setPizza(this.hotel.getText());
	}
